...
sidebar_label: "Advanced worksheet editing options"
sidebar_key: "net-editor-edit-document-edit-excel-advanced-worksheet-editing-options"
sidebar_position: 4
...
source_path: "net/developer-guide/edit-document/edit-excel/advanced-worksheet-editing-options.md"
source_commit: "43d7759259e5e675b78d077fb6e9fc8b768199bb"
source_sha256: "de8db71c0bc23bd8863f221995b199debc9040f1df4f13ae8cd014a17f3e86c6"
---
...
When editing the Spreadsheet documents, the GroupDocs.Editor edits one worksheet (tab) at time. The edited worksheet is specified via <a href="https://reference.groupdocs.com/editor/net/groupdocs.editor.options/spreadsheeteditoptions/worksheetindex/" target="_blank" rel="nofollow noopener noreferrer">`SpreadsheetEditOptions.WorksheetIndex`</a> property. So, when the Spreadsheet document is loaded to the <a href="https://reference.groupdocs.com/editor/net/groupdocs.editor/editor/" target="_blank" rel="nofollow noopener noreferrer">`Editor`</a> class, the user creates a <a href="https://reference.groupdocs.com/editor/net/groupdocs.editor.options/spreadsheeteditoptions/" target="_blank" rel="nofollow noopener noreferrer">`SpreadsheetEditOptions`</a> instance, specifies a desired worksheet index, and calls the `Editor.Edit` method. As a result, `Editor.Edit` converts the selected worksheet to the editable HTML format and encapsulates it in the <a href="https://reference.groupdocs.com/editor/net/groupdocs.editor/editabledocument/" target="_blank" rel="nofollow noopener noreferrer">`EditableDocument`</a> instance.

Before the <a href="https://releases.groupdocs.com/editor/net/release-notes/2025/groupdocs-editor-for-net-26-6-release-notes/" target="_blank" rel="nofollow noopener noreferrer">version 26.6</a> there were no other options in the <a href="https://reference.groupdocs.com/editor/net/groupdocs.editor.options/spreadsheeteditoptions/" target="_blank" rel="nofollow noopener noreferrer">`SpreadsheetEditOptions`</a> except the `ExcludeHiddenWorksheets` that only defines whether or not the hidden worksheets should be taken into account. However, starting from the version 26.6, two new options (public properties) were added: `MergeEmptyAdjacentCells` and `ExportBogusRowData` properties, both of `Boolean` type.

At first glance, the worksheets, as they can be seen from MS Excel, and HTML tables may seem to be very similar. However they have more differences than similarities: different formatting, layout, rendering, processing, internal structure, different rules of how their appearance is calculated, and so on. Maybe the most important difference - the Office Open XML Spreadsheets effectively support sparse data. There may be a worksheet with only two non-empty cells: one, for example, is `A1`, while the second is `ZZ1000000000`. Such worksheet size will be less than 1<a href="https://en.wiktionary.org/wiki/kibibyte" target="_blank" rel="nofollow noopener noreferrer">KiB</a> and MS Excel will open it instantly. For HTML tables, which are not able to effectively store such sparse data, such a document will require many hundreds of <a href="https://en.wiktionary.org/wiki/mebibyte" target="_blank" rel="nofollow noopener noreferrer">MiB</a>s and a common browser the most likely will hang on while trying to open it.
...
![Merged empty cells in produced HTML document as it can be seen in a browser debugger](/net/editor/images/Merged-empty-cells-in-browser-debugger.png)
...
![Bogus bottom empty row in produced HTML document as it can be seen in a browser debugger](/net/editor/images/Bogus-bottom-row-in-HTML.png)
...
![Bogus bottom empty row in produced resultant Spreadsheet document as it can be seen in MS Excel](/net/editor/images/Bogus-bottom-row-in-produced-Spreadsheet.png)
...
#### Example
Both these options are very easy to use - they both are boolean and present in the <a href="https://reference.groupdocs.com/editor/net/groupdocs.editor.options/spreadsheeteditoptions/" target="_blank" rel="nofollow noopener noreferrer">`SpreadsheetEditOptions`</a> class. Code below shows loading an input document, creating a "SpreadsheetEditOptions" instance, tuning these options, and editing the document, where the <a href="https://reference.groupdocs.com/editor/net/groupdocs.editor/editabledocument/" target="_blank" rel="nofollow noopener noreferrer">`EditableDocument`</a> instance is created.
