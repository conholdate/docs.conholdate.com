...
sidebar_label: "Network access and data privacy"
sidebar_key: "net-parser-network-access-and-data-privacy"
sidebar_position: 6
...
source_path: "net/getting-started/network-access-and-data-privacy.md"
source_commit: "e3b3a2df0663abc09348cedab04e0e649de093a4"
source_sha256: "32d91685dc09d01c7c0675d8fcd7e1d447c2e4580c6614cb16cce0f107dbfa71"
---
...
As stated in the <a href="https://about.groupdocs.com/security/customer-data-and-security/" target="_blank" rel="nofollow noopener noreferrer">GroupDocs Customer Data and Security policy</a>:
...
This matches the <a href="https://about.groupdocs.com/legal/terms-of-use/" target="_blank" rel="nofollow noopener noreferrer">GroupDocs Terms of Use</a>:
...
The [Parser(Uri)](/net/parser/developer-guide/advanced-usage/loading/load-document-from-url/) constructor overloads instruct GroupDocs.Parser to download the document itself:
...
The [Parser(EmailConnection)](/net/parser/developer-guide/advanced-usage/extract-data-from-various-formats/extract-data-from-emails/extract-emails-from-remote-server-via-pop-imap-or-exchange-web-services-protocols/) overloads connect to a POP3, IMAP, or Exchange Web Services server that **you** specify via `EmailPopConnection`, `EmailImapConnection`, or `EmailEwsConnection`. Likewise, [Parser(DbConnection)](/net/parser/developer-guide/advanced-usage/extract-data-from-databases/) reaches whatever server the connection you supply points at, which may be remote.
...
If - and only if - you activate a [metered license](/net/parser/getting-started/evaluation-limitations-and-licensing/) with `SetMeteredKey`, the library contacts the licensing and billing endpoint. The key is validated over the network when you set it, and consumption data is uploaded on a recurring basis thereafter.
...
[ExternalResourceHandler](/net/parser/developer-guide/advanced-usage/loading/handle-loading-of-external-resources/), supplied through <a href="https://reference.groupdocs.com/net/parser/groupdocs.parser.options/parsersettings" target="_blank" rel="nofollow noopener noreferrer">ParserSettings</a>, lets you inspect each resource URI before it is fetched and skip it by setting `args.Skipped = true`.


:::danger
As of version 26.6.1 the handler is honoured on **image extraction from HTML and MHTML only**. On every other path - including `GetText`, `GetFormattedText` and `GetStructure`, email bodies, linked images in Word processing documents, and `INCLUDEPICTURE` fields - <a href="https://reference.groupdocs.com/net/parser/groupdocs.parser.options/externalresourcehandler/methods/onloading" target="_blank" rel="nofollow noopener noreferrer">OnLoading</a> is never invoked and the request is still issued.
...
**3. Do not use `Parser(Uri)`.** Load from a local path or a `Stream` instead. If your application must accept URLs, download them in your own code where you can apply your own validation and your own timeout, then hand the resulting `Stream` to [Parser(Stream)](/net/parser/developer-guide/advanced-usage/loading/load-document-from-stream/).
...
*   [Handle loading of external resources](/net/parser/developer-guide/advanced-usage/loading/handle-loading-of-external-resources/)
*   [Load document from url](/net/parser/developer-guide/advanced-usage/loading/load-document-from-url/)
*   [Load document from stream](/net/parser/developer-guide/advanced-usage/loading/load-document-from-stream/)
*   [Extract data from Emails](/net/parser/developer-guide/advanced-usage/extract-data-from-various-formats/extract-data-from-emails/)
*   [Evaluation limitations and licensing](/net/parser/getting-started/evaluation-limitations-and-licensing/)
*   <a href="https://about.groupdocs.com/security/customer-data-and-security/" target="_blank" rel="nofollow noopener noreferrer">GroupDocs Customer Data and Security</a>
